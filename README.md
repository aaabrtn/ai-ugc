...
3. **Polls automatically in the background** (`GET /api/v1/jobs/recordInfo`) with gentle exponential backoff (starting ~3s, capped at 15s, matching KIE's own guidance) — independent of which tab or page is open, so switching tabs or navigating back to the Generator's blank form never loses track of an in-flight generation. Polling gives up after 15 minutes with a clear timeout message rather than spinning forever.
