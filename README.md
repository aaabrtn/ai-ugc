...
1. **Hosts reference photos on KIE's own File Upload API.** KIE's video model can't reach anything on `localhost`, so before submitting, the character's setting photos and the product's photos are uploaded to KIE's own storage (`POST /api/file-stream-upload` at `kieai.redpandaai.co`, using the same `KIE_API_KEY`), which returns a public URL KIE's video endpoint can fetch from. This replaced an earlier version that used catbox.moe, a third-party anonymous host — that worked in testing but failed on a real submission with "Image fetch failed. Check access settings or use our File Upload API instead," so the app now uses KIE's own upload path instead, avoiding a dependency on a third-party host's reliability. Uploaded files are temporary (KIE auto-deletes them within roughly a day), which is fine since the URL is only needed for the one immediate request. Identity photos aren't uploaded this way at all — identity is carried entirely by the character's KIE `character_id` instead (see below).
